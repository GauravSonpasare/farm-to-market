...
import {
  LayoutDashboard, Sprout, ListOrdered, ShoppingCart, Receipt,
  CloudSun, FileBadge, TrendingUp, LogOut, Menu, X, User,
} from "lucide-react";
import { NotificationBell } from "../../../components/layout/notification-bell";

const navItems = [
  { href: "/farmer",          label: "Home",          icon: LayoutDashboard },
  { href: "/farmer/upload",   label: "Upload Crop",   icon: Sprout           },
  { href: "/farmer/listings", label: "My Listings",   icon: ListOrdered      },
  { href: "/farmer/orders",   label: "Orders",        icon: ShoppingCart     },
  { href: "/farmer/payments", label: "Payments",      icon: Receipt          },
...
export function FarmerLayout({ children }: { children: ReactNode }) {
  ...
  const initial = ((user as any)?.email?.charAt(0) || "F").toUpperCase();
  const name    = (user as any)?.name || (user as any)?.email?.split("@")[0] || "Farmer";
  ...
  const SidebarContent = () => (
    <div className="ag-sidebar w-64 flex-shrink-0 flex flex-col" style={{ height: "100vh" }}>
      ...
      <div className="px-5 py-6" style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "#e9c46a" }}>Farmer Panel</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full mt-0.5 inline-block"
              style={{ background: "rgba(233,196,106,0.2)", color: "#e9c46a" }}>🌿 Farmer</span>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        {navItems.map((item, i) => {
          const isActive = item.href === "/farmer" ? location === "/farmer" : location.startsWith(item.href);
      ...
      <main className="ag-main flex-1">
        <header className="hidden md:flex sticky top-0 z-10 items-center justify-between px-8 py-3"
          ...
          <h1 className="font-bold text-sm capitalize" style={{ color: "#1b2e1e" }}>
            {navItems.find(n => n.href === location)?.label || "Farmer Dashboard"}
          </h1>
          <div className="flex items-center gap-3">
            <NotificationBell />
          </div>
