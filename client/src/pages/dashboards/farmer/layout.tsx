...
export function FarmerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 overflow-hidden text-slate-900">
      ...
      <motion.aside
        initial={{ x: -300 }}
        animate={{ x: isSidebarOpen ? 0 : 0 }} // Responsive handles the actual display logic below
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-green-900 text-green-50 shadow-2xl transition-transform transform lg:translate-x-0 lg:static lg:flex lg:flex-col ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="p-6 flex items-center justify-between xl:justify-center border-b border-green-800">
          <Link href="/farmer">
            <span className="text-2xl font-bold flex items-center gap-2 cursor-pointer">
              <Sprout className="h-6 w-6 text-green-400" />
              <span className="hidden lg:block">Farmer Panel</span>
              ...
              <Link key={item.href} href={item.href}>
                <a
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all cursor-pointer ${
                    isActive
                      ? "bg-green-700/50 text-white font-medium shadow-inner"
                      : "text-green-200 hover:bg-green-800/50 hover:text-green-50"
                  }`}
                >
